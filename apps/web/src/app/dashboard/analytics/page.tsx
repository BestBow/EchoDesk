    ...
    <div className="max-w-4xl mx-auto">
      ...
      <div className="grid grid-cols-3 gap-4 mb-6">
        {[
          ...
          { label: 'Questions', value: analytics?.questionBreakdown?.length ?? 0 },
        ].map((stat) => (
      ...
      <div className="bg-card border rounded-lg p-5 mb-6">
        ...
        {aiSummary ? (
          ...
          <p className="text-sm text-muted-foreground">
            Click &quot;Generate summary&quot; to get an AI-powered analysis of your text responses.
          </p>
        )}
      </div>
      ...
      {analytics?.questionBreakdown?.map((q: any) => (
        <div key={q.questionId} className="bg-card border rounded-lg p-5 mb-4">
          ...
          {['SINGLE_CHOICE', 'MULTI_CHOICE', 'RATING', 'NPS'].includes(q.type) &&
            typeof q.data === 'object' && !Array.isArray(q.data) && (
              <ResponsiveContainer width="100%" height={160}>
                <BarChart data={Object.entries(q.data).map(([name, value]) => ({ name, value }))}>
                  <XAxis dataKey="name" tick={{ fontSize: 11 }} />
                  <YAxis tick={{ fontSize: 11 }} allowDecimals={false} />
                  <Tooltip />
                  <Bar dataKey="value" radius={[4, 4, 0, 0]}>
                    {Object.keys(q.data).map((_: any, i: number) => (
          ...
          {['SHORT_TEXT', 'LONG_TEXT'].includes(q.type) && Array.isArray(q.data) && (
            <div className="space-y-2">
              {q.data.slice(0, 5).map((answer: string, i: number) => (
                <p key={i} className="text-sm text-muted-foreground bg-muted rounded-md px-3 py-2">
                  {String(answer)}
                </p>
              ))}
              {q.data.length > 5 && (
                <p className="text-xs text-muted-foreground">+{q.data.length - 5} more responses</p>
              )}
            </div>
          )}
        </div>
